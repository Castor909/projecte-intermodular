...
## 🔴 Нужно исправить (баги / рассинхрон)

### Цена со скидкой в корзине и на странице альбома
`discountPercent` учитывается только в AlbumCard и SpecialOffers. На `/album/:id` и в корзине всегда показывается полная цена — рассинхрон.
- Страница альбома: показывать зачёркнутую оригинальную и новую цену если `discountPercent > 0`
- Корзина: добавлять товар по сниженной цене, отображать обе цены в списке
- PaymentPage: итог считать по сниженным ценам
